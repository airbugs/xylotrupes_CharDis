...
#the resulted lik is not sig diff among models
...
for(i in 1:1000){
	...
	if(temp.tree$states['BE']!=temp.tree$states['CAMP']){BECAMPcount <- BECAMPcount + 1};	
...
hist(char.dis.count, xlim = c(0,10), xlab = 'Expected number of sympatric taxa \n exhibiting different male phenotypes', main = NULL);
